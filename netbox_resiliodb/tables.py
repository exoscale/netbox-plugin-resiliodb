...
class DeviceResilioTable(NetBoxTable):
    name = tables.Column(linkify=True)
    device_role = tables.Column(linkify=True, accessor="role")

    def get_resilio_type(self, record):
        if not record.role:
            return None
        mapping = DeviceRoleLCATypeMapping.objects.filter(
            device_role=record.role
        ).first()
        return mapping.lca_type if mapping else None

    resilio_type = tables.Column(verbose_name="Resilio Type", accessor="get_lca_type")
    site = tables.Column(linkify=True)

    def get_region(self, record):
        return record.site.region if record.site else None

    region = tables.Column(accessor="site__region", linkify=True)

    has_lca_params = tables.Column(
        verbose_name="Has LCA Params", accessor="has_lca_params"
    )

    device_type_has_lca_params = tables.Column(
        verbose_name="DeviceType Has LCA Params", accessor="device_type__has_lca_params"
    )

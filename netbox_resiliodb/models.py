...
from dcim.models import Device, DeviceRole, DeviceType, Region, Site
...
class DeviceRoleLCATypeMapping(NetBoxModel):
    ...
    device_role = models.ForeignKey(
        DeviceRole, on_delete=models.CASCADE, related_name="lca_mappings"
    )
    ...
    def get_absolute_url(self):
        return reverse(
            "plugins:netbox_resiliodb:devicerolelcatypemapping", args=[self.pk]
        )
...
class LCAImpactData(NetBoxModel):
    ...
    device = models.OneToOneField(
        to=Device, on_delete=models.CASCADE, related_name="lca_impact_data"
    )
    ...
    def is_outdated(self):
        ...
        try:
            ...
            current_hash = client._compute_hash(device_params["lca_type"], payload)
...
class LCAImpactIndicatorValue(NetBoxModel):
    ...
    impact_data = models.ForeignKey(
        LCAImpactData, on_delete=models.CASCADE, related_name="indicator_values"
    )
    ...
    def get_absolute_url(self):
        return reverse(
            "plugins:netbox_resiliodb:lcaimpactindicatorvalue", args=[self.pk]
        )

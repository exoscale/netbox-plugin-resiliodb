...
from ..models import LCACache, PluginSettings
...
class ResilioDBClient:
    ...
    def _ensure_valid_token(self):
        ...
        if (
            ...
            and self.settings.access_token_expiry
        ...
        try:
            ...
            self.settings.access_token_expiry = now + timedelta(
                hours=23
            )  # 23 hours to be safe
            self.settings.save()
    ...
    def _cache_response(self, hash_value: str, response_data: dict) -> LCACache:
        ...
        """
        return LCACache.objects.create(hash=hash_value, request_payload=response_data)
    ...
    def get_footprint(self, lca_type: str, payload: dict) -> dict:
        ...
        try:
            ...
            response_data = {"_raw_data": response.json()}
    ...
    def get_countries(self) -> list:
        ...
        try:
            ...
            response = session.get(
                f"{self.base_url}/api/countries", headers=self._get_headers()
            )
            response.raise_for_status()

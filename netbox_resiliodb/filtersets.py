...
class DeviceResilioFilterSet(NetBoxModelFilterSet):
    has_lca_params = django_filters.BooleanFilter(
        method="filter_has_lca_params", label="Has LCA Parameters"
    )
    ...
    def filter_lca_impact_status(self, queryset, name, value):
        if value == "Missing":
            return queryset.filter(
                pk__in=[
                    d.pk for d in queryset if d.get_lca_impact_status() == "Missing"
                ]
            )
        elif value == "Current":
            return queryset.filter(
                pk__in=[
                    d.pk for d in queryset if d.get_lca_impact_status() == "Current"
                ]
            )
        elif value == "Outdated":
            return queryset.filter(
                pk__in=[
                    d.pk for d in queryset if d.get_lca_impact_status() == "Outdated"
                ]
            )
        return queryset

...
def render_json_as_table(json_data):
    ...
    def render_dict_as_table(d):
        ...
        for key, value in d.items():
            ...
            rows.append(
                f'<tr><th class="text-start">{key}</th><td>{formatted_value}</td></tr>'
            )
        return format_html(
            '<table class="table table-hover">{}</table>', format_html("".join(rows))
        )
    ...
    def render_list_as_table(lst):
        ...
        for i, value in enumerate(lst):
            formatted_value = render_value(value)
            rows.append(
                f'<tr><th class="text-start">{i}</th><td>{formatted_value}</td></tr>'
            )
        return format_html(
            '<table class="table table-hover">{}</table>', format_html("".join(rows))
        )
...
def content_type_parameters(obj):
    ...
    return LCAParams.objects.filter(content_type=content_type, object_id=obj.pk).first()

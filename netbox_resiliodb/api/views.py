...
class LCAImpactDataViewSet(NetBoxModelViewSet):
    ...
    def inventory_export(self, request):
        ...
        if request.query_params.get("select_all") == "true":
            ...
            if not selected_devices:
                return Response(
                    {"error": "No devices selected"}, status=status.HTTP_400_BAD_REQUEST
                )
        ...
        for device in server_devices:
            ...
            lifespan = usage.get(
                "duration_of_use_hour",
                settings.default_usage_period_hours if settings else 43800,
            )
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="devices_inventory_{timestamp}.zip"'
        )
        return response
    ...
    def csv_export(self, request):
        ...
        impact_data_list = (
            models.LCAImpactData.objects.filter(device__in=queryset)
            .select_related("device")
            .prefetch_related("indicator_values__indicator")
        )
        ...
        for impact_data in impact_data_list:
            ...
            for step in steps:
                ...
                for indicator_code in header[4:]:  # Skip hw_id, site, role, and lc_step
                    value = 0
                    indicator_value = indicator_values.filter(
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="devices_impact_{timestamp}.csv"'
        )
        return response
